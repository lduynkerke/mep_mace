...
def parse_coords(file_path):
    ...
    :type file_path: stri
    :return: A tuple containing the list of atomic coordinates and the atom count.
    :rtype: tuple[list[tuple[str, float, float, float]], int]
    """
    coords = []
    with open(file_path, 'r') as f:
        lines = f.readlines()
...
def process_folders(root_folder, base_name, start, end, increment, lattice, atomic_numbers):
    """
    ...
    for i in range(start, end + 1, increment):
        ...
        coords, atom_count = parse_coords(coords_file)
